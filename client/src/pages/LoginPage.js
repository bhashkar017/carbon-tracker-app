...
const LoginPage = ({ loadUser }) => {
  ...
  const onSubmit = async e => {
    e.preventDefault();
    try {
      const res = await axios.post('https://carbon-tracker-api-bhashkar.onrender.com/api/users/login', { email, password });
      ...
      <div className="w-full max-w-md bg-white rounded-lg shadow-md p-8">
        ...
        <form onSubmit={onSubmit}>
          ...
          <div className="mb-6">
            ...
            <input className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline" type="password" name="password" value={password} onChange={onChange} required />
          </div>
